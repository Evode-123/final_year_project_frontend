...
const ManagerDashboard = () => {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    drivers: { total: 0, active: 0, onLeave: 0 },
    ...
    recentActivities: [],
    upcomingTrips: [],
    operationalHealth: { status: 'good', issues: [] }
  });
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [
        ...
        transportApiService.getAllDrivers().catch(() => []),
        ...
        transportApiService.getDailyChecksDashboard().catch(() => ({}))
      ]);
      ...
      if (maintenanceVehicles > 0) {
        operationalIssues.push(`${maintenanceVehicles} vehicle(s) in maintenance`);
      }
      if (vehicleInspections?.overdueCount > 0) {
        operationalIssues.push(`${vehicleInspections.overdueCount} overdue inspection(s)`);
      }
      if (dailyChecks?.unreviewedProblemsCount > 0) {
        operationalIssues.push(`${dailyChecks.unreviewedProblemsCount} unreviewed daily check(s)`);
      }
      if (incidentStats?.criticalIncidents > 0) {
        operationalIssues.push(`${incidentStats.criticalIncidents} critical incident(s)`);
      }
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      {dashboardData.operationalHealth.status !== 'good' && (
        <div className={`p-4 rounded-xl border-2 ${
          dashboardData.operationalHealth.status === 'critical' 
            ? 'bg-red-50 border-red-200' 
            : 'bg-yellow-50 border-yellow-200'
        }`}>
          <div className="flex items-start gap-3">
            <AlertTriangle className={`w-6 h-6 flex-shrink-0 ${
              dashboardData.operationalHealth.status === 'critical' ? 'text-red-600' : 'text-yellow-600'
            }`} />
            <div>
              <p className={`font-semibold ${
                dashboardData.operationalHealth.status === 'critical' ? 'text-red-800' : 'text-yellow-800'
              }`}>
                Operational Attention Required
              </p>
              <ul className={`mt-2 space-y-1 text-sm ${
                dashboardData.operationalHealth.status === 'critical' ? 'text-red-700' : 'text-yellow-700'
              }`}>
                {dashboardData.operationalHealth.issues.map((issue, i) => (
                  <li key={i}>• {issue}</li>
                ))}
              </ul>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={dashboardData.drivers.active}
          subtitle={`of ${dashboardData.drivers.total} drivers ready`}
          trend="3%"
          trendUp={true}
          color="blue"
        />
        ...
        <StatCard
          ...
          subtitle={`${dashboardData.packages.arrived} arrived today`}
          trend="2%"
          trendUp={false}
          color="purple"
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard
          ...
          subtitle={`${dashboardData.incidents.critical} critical priority`}
          color="red"
        />
        <StatCard
          icon={MessageSquare}
          ...
          subtitle={`${dashboardData.feedback.positive} positive reviews`}
          color="yellow"
        />
        <StatCard
          icon={Shield}
          title="Safety Compliance"
          value={`${Math.round(((dashboardData.vehicles.total - dashboardData.vehicles.maintenance) / dashboardData.vehicles.total) * 100) || 0}%`}
          subtitle={`${dashboardData.vehicles.maintenance} under maintenance`}
          color="indigo"
        />
      </div>
          ...
          <div className="space-y-4">
            {dashboardData.upcomingTrips.length > 0 ? (
              dashboardData.upcomingTrips.map((trip, i) => (
                <div key={i} className="p-4 bg-gradient-to-r from-blue-50 to-gray-50 rounded-xl border border-blue-100">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-gray-800 rounded-2xl shadow-xl p-6 text-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div className="text-center">
            <p className="text-blue-100 text-sm mb-1">Weekly Bookings</p>
            <p className="text-3xl font-bold">{dashboardData.bookings.thisWeek}</p>
            <p className="text-blue-200 text-xs mt-1">Estimated this week</p>
          </div>
          <div className="text-center">
            <p className="text-blue-100 text-sm mb-1">Fleet Efficiency</p>
            <p className="text-3xl font-bold">{Math.round((dashboardData.vehicles.available / dashboardData.vehicles.total) * 100) || 0}%</p>
            <p className="text-blue-200 text-xs mt-1">Vehicles operational</p>
          </div>
          <div className="text-center">
            <p className="text-blue-100 text-sm mb-1">Driver Utilization</p>
            <p className="text-3xl font-bold">{Math.round((dashboardData.drivers.active / dashboardData.drivers.total) * 100) || 0}%</p>
            <p className="text-blue-200 text-xs mt-1">Active drivers</p>
          </div>
          <div className="text-center">
            <p className="text-blue-100 text-sm mb-1">Service Quality</p>
            <p className="text-3xl font-bold">{Math.round((dashboardData.feedback.positive / dashboardData.feedback.total) * 100) || 0}%</p>
            <p className="text-blue-200 text-xs mt-1">Positive feedback</p>
          </div>
        </div>
      </div>
    </div>
