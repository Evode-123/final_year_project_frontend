...
import { 
  Package as PackageIcon, Search, Truck, CheckCircle, MapPin,
  AlertCircle, Loader, Filter, Send, Inbox, Home
} from 'lucide-react';
...
const MyPackagesPanel = () => {
  ...
  useEffect(() => { loadMyPackages(); loadStatistics(); }, []);
  ...
  const handleTrackPackage = async (e) => {
    ...
    try {
      setSearching(true); setError(''); setSearchedPackage(null);
  ...
  const getStatusIcon = (status) => {
    switch (status?.toUpperCase()) {
      ...
      case 'ARRIVED':    return <MapPin className="w-5 h-5 text-yellow-600" />;
      case 'COLLECTED':  return <CheckCircle className="w-5 h-5 text-green-600" />;
      case 'CANCELLED':  return <AlertCircle className="w-5 h-5 text-red-600" />;
      default:           return <PackageIcon className="w-5 h-5 text-gray-600" />;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
          <Search className="w-5 h-5 text-blue-600" /> Quick Track Package
        </h3>
        <form onSubmit={handleTrackPackage} className="flex gap-3">
          ...
          />
          <button type="submit" disabled={searching || !trackingNumber.trim()}
            className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center gap-2">
            {searching ? <><Loader className="w-5 h-5 animate-spin" /> Tracking...</> : <><Search className="w-5 h-5" /> Track</>}
          </button>
        </form>
        ...
        {searchedPackage && (
          <div className="mt-4 bg-white border-2 border-blue-300 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-gray-600">From:</span><p className="font-semibold">{searchedPackage.senderNames}</p></div>
              <div><span className="text-gray-600">To:</span><p className="font-semibold">{searchedPackage.receiverNames}</p></div>
              <div><span className="text-gray-600">Route:</span><p className="font-semibold">{searchedPackage.origin} → {searchedPackage.destination}</p></div>
              <div><span className="text-gray-600">Travel Date:</span><p className="font-semibold">{new Date(searchedPackage.travelDate).toLocaleDateString()}</p></div>
              {searchedPackage.senderAddress && (
                <div><span className="text-gray-600">Sender Address:</span><p className="font-semibold">{searchedPackage.senderAddress}</p></div>
              )}
              {searchedPackage.receiverAddress && (
                <div><span className="text-gray-600">Receiver Address:</span><p className="font-semibold">{searchedPackage.receiverAddress}</p></div>
              )}
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div><p className="text-sm text-gray-600">Sent Packages</p><p className="text-2xl font-bold text-gray-800">{statistics.totalSent}</p></div>
                <Send className="w-10 h-10 text-blue-400" />
              </div>
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div><p className="text-sm text-gray-600">In Transit</p><p className="text-2xl font-bold text-blue-600">{statistics.sentInTransit + statistics.receivedInTransit}</p></div>
                <Truck className="w-10 h-10 text-blue-400" />
              </div>
            </div>
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div><p className="text-sm text-gray-600">Arrived</p><p className="text-2xl font-bold text-yellow-600">{statistics.sentArrived + statistics.receivedArrived}</p></div>
                <MapPin className="w-10 h-10 text-yellow-400" />
              </div>
            </div>
            <div className="bg-green-50 border border-green-200 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div><p className="text-sm text-gray-600">Collected</p><p className="text-2xl font-bold text-green-600">{statistics.sentCollected + statistics.receivedCollected}</p></div>
      ...
      <div className="flex items-center gap-4 border-b border-gray-200">
        <button onClick={() => setActiveTab('sent')}
          className={`px-4 py-2 font-medium transition-colors ${activeTab === 'sent' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-800'}`}>
          <div className="flex items-center gap-2"><Send className="w-4 h-4" /> Sent Packages ({sentPackages.length})</div>
        </button>
        <button onClick={() => setActiveTab('received')}
          className={`px-4 py-2 font-medium transition-colors ${activeTab === 'received' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-800'}`}>
          <div className="flex items-center gap-2"><Inbox className="w-4 h-4" /> Received Packages ({receivedPackages.length})</div>
        </button>
      </div>
      ...
      {filteredPackages.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-xl">
          ...
          <p className="text-gray-500 text-sm mt-2">
            {statusFilter !== 'ALL' ? `No ${statusFilter.toLowerCase()} packages` : `No ${activeTab} packages yet`}
          </p>
        </div>
      ) : (
            ...
            <div key={pkg.id}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    {pkg.senderAddress && (
                      <p className="text-xs text-gray-500 flex items-start gap-1 mt-1">
                        <Home className="w-3 h-3 mt-0.5 flex-shrink-0" /> {pkg.senderAddress}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {pkg.receiverAddress && (
                      <p className="text-xs text-gray-500 flex items-start gap-1 mt-1">
                        <Home className="w-3 h-3 mt-0.5 flex-shrink-0" /> {pkg.receiverAddress}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-800">{pkg.origin} → {pkg.destination}</p>
                    <p className="text-sm text-gray-600">{new Date(pkg.travelDate).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
              ...
              {pkg.actualArrivalTime && pkg.packageStatus === 'ARRIVED' && (
                <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                  ...
                  <p className="text-sm text-yellow-800 mt-1">Ready for collection</p>
                </div>
              )}
            </div>
          ))}
